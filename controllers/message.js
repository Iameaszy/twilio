  ...
  function processMessage(subscriber) {
    ...
    if (msg.match(/subscribe/ig) || msg.match(/stop/ig)) {
      ...
      subscriber.save(function (err) {
        ...
        if (!subscriber.subscribed && msg.match(/stop/ig)) {
          responseMessage =
      ...
      const responseMessage =
        ...
        'available commands are: subscribe or stop';
...
exports.sendMessages = function (request, response) {
  ...
  Subscriber.find({
    ...
    .then((subscribers) => {
      messageSender
        .sendMessageToSubscribers(subscribers, message, imageUrl)
        .then(() => {
